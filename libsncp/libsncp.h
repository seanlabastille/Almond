...
void sncp_slave_register(uint8 class, &sleep_callback);
void sncp_send_package(sncp_Packet_Type t, &data, &error);
void sncp_receive_package();
#endif
#ifdef sncp_SLAVE
void sncp_slave_init(uint8[16] *device_name, &sleep_callback);
void sncp_slave_register_sensor(uint8 class, &sleep_callback);
void sncp_slave_register_actuator(uint8 class, &sleep_callback);
#endif